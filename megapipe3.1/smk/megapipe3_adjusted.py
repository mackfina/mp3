...
with open(config["logs_analysis"]+"ids.txt", "r") as file:
...
rule all:
    input:
        ...
        expand(config["output"]+"{sample}/bam/{sample}.duprem.bam.bai", sample=samples),
        expand(config["output"]+"{sample}/fast-lineage-caller/{sample}.lineage", sample=samples),
        expand(config["output"]+"{sample}/spades/scaffolds.fasta", sample=samples),
        expand(config["output"]+"{sample}/kraken/{sample}.krkn", sample=samples)
...
rule spades_assemble:
    ...
    output:
        asm1=config["output"]+"{sample}/spades/scaffolds.fasta",
        dir=config["output"]+"{sample}/spades",
    log:
...
rule remove_duplicates:
    ...
    output:
        outfile=config["output"]+"{sample}/bam/{sample}.duprem.bam", metrics=config["temp_dir"]+"results/{sample}/bam/{sample}.metrics"
    log:
...
rule calculate_depth:
    input:
        config["output"]+"{sample}/bam/{sample}.duprem.bam"
    output:
        config["output"]+"{sample}/depth/{sample}.depth.gz", config["output"]+"{sample}/depth/{sample}_depth_OK"
    log:
        config["logs_analysis"]+"{sample}/calc_depth.txt"
    shell:
        """
        samtools depth -a {input} > /n/data1/hms/dbmi/farhat/rollingDB/MIC-ML_output/Wadsworth/{wildcards.sample}/depth/{wildcards.sample}.depth 2> {log}
        ./bin/check_depth.py /n/data1/hms/dbmi/farhat/rollingDB/MIC-ML_output/Wadsworth/{wildcards.sample}/depth/{wildcards.sample}.depth >> {log} 2>&1
        gzip /n/data1/hms/dbmi/farhat/rollingDB/MIC-ML_output/Wadsworth/{wildcards.sample}/depth/{wildcards.sample}.depth >> {log} 2>&1
        """

rule indexing_bam:
    input:
        bam=config["output"]+"{sample}/bam/{sample}.duprem.bam", depth_ok=config["output"]+"{sample}/depth/{sample}_depth_OK"
    output:
        config["output"] + "{sample}/bam/{sample}.duprem.bam.bai"
    log:
...
rule variant_calling:
    input:
        bam=config["output"]+"{sample}/bam/{sample}.duprem.bam",depth_ok=config["output"]+"{sample}/depth/{sample}_depth_OK",  bai=config["output"]+"{sample}/bam/{sample}.duprem.bam.bai"
    output:
        config["output"]+"{sample}/pilon/{sample}.vcf", config["output"]+"{sample}/pilon/{sample}.fasta", config["output"]+"{sample}/pilon/{sample}_full.vcf.gz"
    log:
        config["logs_analysis"]+"{sample}/variant_calling.txt"
    shell:
        """
        pilon -Xmx32G --genome {config[path_ref_genome]} --bam {input.bam} --output /n/data1/hms/dbmi/farhat/rollingDB/MIC-ML_output/Wadsworth/{wildcards.sample}/pilon/{wildcards.sample} --variant > {log} 2>&1
        mv /n/data1/hms/dbmi/farhat/rollingDB/MIC-ML_output/Wadsworth/{wildcards.sample}/pilon/{wildcards.sample}.vcf /n/data1/hms/dbmi/farhat/rollingDB/MIC-ML_output/Wadsworth/{wildcards.sample}/pilon/{wildcards.sample}_full.vcf >> {log} 2>&1
        ./bin/vcf_cutter.py /n/data1/hms/dbmi/farhat/rollingDB/MIC-ML_output/Wadsworth/{wildcards.sample}/pilon/{wildcards.sample}_full.vcf /n/data1/hms/dbmi/farhat/rollingDB/MIC-ML_output/Wadsworth/{wildcards.sample}/pilon/{wildcards.sample}.vcf >> {log} 2>&1
        gzip /n/data1/hms/dbmi/farhat/rollingDB/MIC-ML_output/Wadsworth/{wildcards.sample}/pilon/{wildcards.sample}_full.vcf >> {log} 2>&1
        """

rule lineage_calling:
    input:
        config["output"]+"{sample}/pilon/{sample}.vcf"
    output:
        config["output"]+"{sample}/fast-lineage-caller/{sample}.lineage"
    log:
...
rule kraken:
    ...
    output:
        r1 = config["output"]+"{sample}/kraken/{sample}.krkn",
    shell:
